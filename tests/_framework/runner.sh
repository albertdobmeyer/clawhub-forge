...
for f in "$TESTS_DIR"/*.test.sh; do
  ...
  # If filtering by skill name, only run matching test file
  if [[ -n "$FILTER" ]]; then
...
echo -e "\n${BOLD}Running skill tests${RESET}"
...
for test_file in "${test_files[@]}"; do
  ...
  # Set SKILL variable for tests to use
  SKILL_DIR="$REPO_ROOT/skills/$test_name"
  if [[ -f "$SKILL_DIR/SKILL.md" ]]; then
    export SKILL="$SKILL_DIR/SKILL.md"
  else
    echo -e "  ${YELLOW}SKIP${RESET}: No skill found at skills/$test_name/SKILL.md"
    continue
  fi

  # Reset assertion counters for this file
  ASSERT_PASS=0
  ASSERT_FAIL=0
